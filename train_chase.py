...
import sys
import random
# 确保当前文件夹在模块搜索路径中，能够正确导入 model.py
...
# 命令行参数解析
parser = argparse.ArgumentParser(description='RSANet Training with/without SASPP and EGA')
parser.add_argument('--use_saspp', action='store_true', help='启用SASPP模块')
parser.add_argument('--use_damm', action='store_true', help='启用DAMM(方向感知混合注意力)模块')  # 原来是 --use_ega
parser.add_argument('--use_dropblock', action='store_true', help='启用DropBlock模块')  # 新增
...
use_dropblock = args.use_dropblock  # 新增
use_gcnet = args.use_gcnet
use_damm_spa = args.use_damm_spa
use_damm_ca = args.use_damm_ca
use_damm_pa = args.use_damm_pa
use_damm_dir = args.use_damm_dir
...
if use_damm:
    damm_modules = []
    if use_damm_spa: damm_modules.append('spa')
    if use_damm_ca: damm_modules.append('ca')
    if use_damm_pa: damm_modules.append('pa')
    if use_damm_dir: damm_modules.append('dir')
    if damm_modules:
        modules.append('damm-' + '-'.join(damm_modules))
    else:
        modules.append('damm-none')  # 使用DAMM框架但不使用任何分支
if use_dropblock:
...
if use_damm:  # 原来是 use_ega
    module_info.append('DAMM')
if use_dropblock:
    module_info.append('DropBlock')  # 新增
if use_gcnet:
...
def main():
    ...
    model = RSANet(input_channels=1, start_neurons=start_neurons,
              keep_prob=keep_prob, block_size=block_size,
              use_saspp=use_saspp, use_damm=use_damm,  
              use_dropblock=use_dropblock, use_gcnet=use_gcnet,
              use_damm_spa=use_damm_spa, use_damm_ca=use_damm_ca,
              use_damm_pa=use_damm_pa, use_damm_dir=use_damm_dir).to(device)

    criterion = nn.BCELoss()
    optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    writer = SummaryWriter(log_dir=f'logs/{module_name}')

    weight_path = os.path.join(ckpt_dir, f'RSAN_{module_name}.pth')
